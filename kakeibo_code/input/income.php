...
<body>
    ...
    $db = new PDO_cun();
    $catlist = $db->sql_exe_list("select * from incomecategory");
    ?>
    <h1>入金入力</h1>
    <a href="../index.html"><img src="../setting/93634.png" style="width: 20px;hight: 20px;">戻る</a>
    <br>
    <br>
    <form action="../setting/insert.php" method="POST" name = "income">
    <span>カテゴリー：</span>
   
    <select name="cate" id ="cate">
        <?php
        while($incate = $catlist->fetch()){
            echo "<option value=".$incate['id'].">".$incate['category']."</option>";
        }
    ?>
     </select>
    <br>
    <span>日付:</span><input type="date" id="calender" name="date" ><br>
